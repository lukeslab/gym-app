...
const updateUserExerciseHistory = asyncHandler(async (req, res) => {
    const { id } = req.params
    const { exerciseHistory } = req.body

    const user = await User.findById(id).exec()

    const result = await User.exerciseHistory.push()

    await User.save()
    console.log('User exercise history updated!')
})
